...
| 5.1/5.2 | Enforce request-size limits (431 on excess)                      | SHOULD | ⚠️     | 16 KiB head cap in `server::connection`                                    | Cap exists (F8), but the connection drops with no response; `431` planned (SEC-HTTP-001)  |
| 6.3     | `Content-Length` body framing                                    | MUST   | ✅     | `HttpRequest::from_buffer` (10 MiB cap)                                    | manual; overflow drops the connection, `413` planned (SEC-HTTP-004)                       |
...
| 7.1.1   | Chunk extensions                                                 | MAY    | ❌     |                                                                            | Rejected as malformed (stricter than required, which is fine)                            |
| 7.1.2   | Trailers                                                         | MAY    | ⚠️     |                                                                            | Rejected as malformed (stricter than required, which is fine)                            |
| 9.3     | Persistence: keep-alive by default for HTTP/1.1                  | MUST   | ✅     | `server::connection::handle_connection` loop                               | manual smoke; `Connection: close` honored                                                |
| 9.3     | HTTP/1.0 semantics (`Connection: keep-alive` opt-in)             | SHOULD | ❌     |                                                                            | **F7**: version token parsed, never consulted                                            |
| 9.3.2   | Pipelining                                                       | MUST   | ❌     |                                                                            | **F1 (P0)**: buffered bytes after the head are discarded                                 |
| 6.1     | `Expect: 100-continue`                                           | MUST   | ❌     |                                                                            | Not implemented                                                                          |
| 9.6     | Tear down cleanly on errors                                      | MUST   | ⚠️     | errors propagate, connection closes                                        | closes without response or status (see controls)                                         |
...
- Blocking correctness/security gaps: **F1** (pipelining and body buffering),
  **F4** (CL/TE precedence), missing `Host` validation.
