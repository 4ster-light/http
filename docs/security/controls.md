...
| ------------ | --------------------------------------------------------------------- | ------------------------ | --------- | ------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| SEC-HTTP-001 | Header-size cap                                                       | Header bomb (F8)         | Partial   | 16 KiB cap in `server::connection`; connection drops silently | Respond `431` and close; boundary tests                                                                           |
...
| SEC-HTTP-003 | Reject `Content-Length` + `Transfer-Encoding` together; TE precedence | Request smuggling (F4)   | Scheduled | CL path silently wins                                         | Reject or let TE win per RFC 9112 §6.3; conflict tests                                                            |
| SEC-HTTP-004 | Body-size cap                                                         | Memory exhaustion        | Partial   | 10 MiB cap; connection drops silently                         | Respond `413`; limit typed in `Limits`; tests                                                                     |
...
| SEC-HTTP-006 | Path traversal protection                                             | Arbitrary file read      | Partial   | Canonicalize + prefix check                                   | Read via the canonical path (F10 TOCTOU); regression tests for `..`, encoded variants, absolute paths, symlink escape |
| SEC-HTTP-007 | Buffer ownership: no discarded bytes                                  | Framing desync (F1, P0)  | Scheduled | Bytes past the header end discarded                           | Single connection-owned buffer (ADR-0005); pipelining regression test                                             |
...
- The "Partial" rows that end a connection without a protocol response share
  one root cause: parse errors propagate as Rust errors instead of being
  translated into protocol-level replies. The security phase introduces that
  translation layer once, then each control uses it.
