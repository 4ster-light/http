...
Everything here is plain markdown — no build step. (We chose plain markdown over
mdBook: the audience reads these docs on GitHub, where markdown renders
natively, and a zero-tooling docs tree cannot rot. The layout stays compatible
...
One row per RFC requirement; `❌` is allowed and honest — the matrix _is_ the
roadmap.
...
- [security/controls.md](security/controls.md) — the SEC-HTTP-\* / SEC-WS-\*
...
decisions — all defined in [REFACTOR-PLAN.md](../REFACTOR-PLAN.md).
