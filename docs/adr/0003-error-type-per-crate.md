...
- `server::ServerError`: aggregates `Http(#[from])`, `WebSocket(#[from])`, `Io`,
  and application variants (`FileNotFound`, `PortUnavailable`).

## Consequences

- No dependency cycles, ever: errors flow in the same direction as the crate
  graph.
- Error provenance is obvious from the type; matching on application errors in
  `server` is exhaustive.
- Small boilerplate cost at boundaries (`map_err(ServerError::from)`), paid
  consciously.
