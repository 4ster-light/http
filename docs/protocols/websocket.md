...
- and returns the `Sec-WebSocket-Key` value when all match.
...
**Not yet validated (F11 → SEC-WS-007):** the method being `GET`, the HTTP
version being ≥ 1.1, and the key's format (base64 of 16 bytes). A request with a
nonsense key currently gets a syntactically valid `101`.
...
- Expects **client-to-server** traffic: unmasked input is rejected
  (`UnmaskedClientFrame`, §5.3). `to_bytes()` does the opposite direction —
  server frames are unmasked with FIN set.
- 7/16/64-bit payload lengths supported. Length bookkeeping is done in `u64` and
  bounds-checked _before_ any `usize` conversion, so a hostile 64-bit length
  cannot truncate into a false "frame complete" on 32-bit targets.
- Control frames with payloads > 125 bytes are rejected (`ControlFrameTooLarge`,
  §5.5).
- Text payloads are UTF-8 validated (`InvalidUtf8`).
- Close codes are validated against the registered ranges
  (`1000..=1003 | 1007..=1011 | 3000..=4999`); anything else →
  `InvalidCloseCode`.
- Unknown/reserved opcodes are mapped to `Close`, so the connection layer
  terminates the connection rather than continuing with a desynchronized stream.
...
- **F5 / SEC-WS-002:** no cap on data-frame payloads — a frame may declare an
  enormous length and force buffering. Max payload with close code 1009 planned.
- **F6 / SEC-WS-003:** RSV bits are ignored (must be 0 without extensions), and
  fragmented control frames are not detected (FIN ignored).
...
- Text → echoed back prefixed with `Echo:`; binary → echoed as-is.
- Client ping → pong with the same payload; pong → clears the liveness flag.
- **Liveness:** a server ping every 30 s; if no pong arrives before the next
  ...
  shuts down.
- Close → reply close and shut down cleanly.
- Incoming bytes buffer across reads; incomplete frames wait for more data.

Timeouts are relative to the ping ticker, not to frame activity — a fully silent
client is dropped after at most ~60 s.
