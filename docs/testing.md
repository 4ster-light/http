...
`http` integration tests (`crates/http/tests/http_tests.rs`):
...
(`crates/websocket/tests/websocket_tests.rs`):
...
`test_websocket_frame_close_with_code`, `test_websocket_frame_ping_pong`.
These cover frame parse/serialize round-trips against the public API, including
the masked-client-frame requirement.
...
- Each security test gets a doc comment with control ID, RFC section, simulated
  attack, and expected behavior. The IDs are greppable from
...
paths in `http` and `websocket` high enough that every compliance-matrix row
has at least one linked test. There is no global percentage gate.
