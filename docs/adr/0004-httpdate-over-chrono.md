...
one call. `chrono` is a large date/time library, and we used about 0% of its
surface. It has also historically dragged in the vulnerable `time 0.1`
dependency (RUSTSEC-2020-0071) unless carefully feature-gated. `httpdate` is a
tiny, single-purpose crate that does exactly HTTP date parsing and formatting.
